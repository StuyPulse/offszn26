 ...
 *
 * <p>
 * Wraps a single dynamically-added CTRE {@code StatusSignal<T>} together
 * with its most recently refreshed value, so it can be logged and replayed
 * by AdvantageKit.
 *
...
public final class LoggedSignal<T> {
    ...
    public LoggedSignal(StatusSignal<T> signal) {
        ...
        if (!getIsSupportedType(value)) {
            this.unsupportedTypeAlert = new Alert(
                "LoggedSignal",
                "Signal '" + key + "' has an unsupported type (" + value.getClass().getName() + "), so it will not be replayable.",
                AlertType.kWarning
            );
